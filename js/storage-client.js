(function () {
  ...
  async function getViewUrl(objectId) {
    const grant = await callStorageApi('download-grant', { objectId });
    return grant.download.url;
  }

  async function downloadObject(objectId) {
    const url = await getViewUrl(objectId);
    window.location.assign(url);
    return { download: { url } };
  }

  async function cloneObject(objectId, purpose, publicRead = false) {
    const result = await callStorageApi('clone', { objectId, purpose, publicRead });
    return result.object;
  }

  async function deleteObject(objectId) {
    return callStorageApi('delete', { objectId });
  }

  window.OdysseyStorage = { uploadFile, listObjects, downloadObject, deleteObject, cloneObject, getViewUrl };
}());
