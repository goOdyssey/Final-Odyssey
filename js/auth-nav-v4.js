...
(function () {
  ...
  function loggedOutHtml() {
    const T = window.OdysseyI18n?.t || ((k) => k);
    return '<a class="nav-login" href="auth.html?mode=login">' + (T('nav_login') || 'Log in') + '</a>' +
      '<a class="nav-cta" href="auth.html?mode=signup">' + (T('nav_signup') || 'Sign up free ✨') + '</a>';
  }
  ...
  async function verifiedAuthIdentity() {
    ...
    // getUser() is the authoritative server-verified identity check. There is
    // no need to perform a second getSession() request before it.
