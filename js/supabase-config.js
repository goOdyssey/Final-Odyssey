(function(){
  ...
  const finalUrl = cleanProjectUrl(fileHasRealConfig ? url : (window.ODYSSEY_SUPABASE_URL || fromStorage.url || url));
  ...
  window.OdysseySupabaseConfig = {
    ...
    save(nextUrl, nextAnonKey){
      localStorage.setItem('ODYSSEY_SUPABASE_URL', cleanProjectUrl(nextUrl));
