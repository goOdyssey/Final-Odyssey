...
For the current production identity/publishing build, run the migrations in numeric order through `010_member_code_allocator_and_comment_identity.sql`. In particular, `007` introduces the 8-digit member identity, corrected `009` finalizes the atomic allocator, legacy student-code mirror, publishing limits, promo codes, and comment identity, and `010` is an idempotent hardening pass for the same allocator/identity layer.
